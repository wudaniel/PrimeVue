  ...
  <div>
    ...
    <div v-else>
      ...
      <div v-else-if="detailData.length > 0">
        <Card class="mt-3">
          <template #title> 詳細資料 (ID: {{ selectedItemId }}) </template>
          <template #content>
            <!-- 模擬 v-list 顯示 -->
...
type ChipSeverity =
  ...
  | "secondary"
  | "contrast"
  | null; // Chip 的 Severity 類型

const getStatusSeverity = (status: number): ChipSeverity => {
  switch (status) {
    case 0:
      return "secondary"; // 未開案 -> 次要 (灰色系)
    case 1:
      return "success"; // 已開案 -> 成功
    case 2:
      return "warning"; // 不開案 -> 警告
    case 3:
      return "contrast"; // 已結案 -> 使用 contrast 或其他定義的深色系
    default:
      return null; // 未知 -> 預設樣式
  }
};
...
// --- 方法 (API 呼叫邏輯不變) ---
const handleIdClick = async (item: any) => {
  // 使用 any 或更精確的行類型
  ...
  try {
    const apitypeMap: { [key: number]: string } = {
      1: "general",
      2: "arrival",
    };
    const getApiTypeText = (type: number): string =>
      apitypeMap[type] || "unknown"; // 修正函式名稱避免衝突
